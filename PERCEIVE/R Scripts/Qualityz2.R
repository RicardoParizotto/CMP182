...
#Quality Z2
myFormula <- QUALITYZ2 ~ DELAY + TCPTP + TILE
my.data_rpart <- rpart(myFormula, data = train, method = "anova", control=rpart.control(cp=0.00001, minsplit = 2))

barres=test[,"QUALITYZ2", drop=FALSE]
...
my.data_prune <- prune(my.data_rpart, cp = 1.6113e-04 )
print(my.data_prune)
plot(my.data_prune)
...
write.csv2(DEXfat_pred, "qualz2predictedR1112.txt")
write.csv2(test$QUALITYZ2, "qualz2testR1112.txt")
