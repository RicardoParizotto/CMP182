...
#STALL
myFormulaStall <- STALL ~ DELAY + TCPTP + TILE
...
my.data_prune <- prune(my.data_rpart, cp = 1.5294e-04)
my.data_prune <- prune(my.data_rpart, cp = 2.2985e-04)
print(my.data_prune)
plot(my.data_prune)
text(my.data_prune, cex=0.75)

# TEST is a fraction of the entire data
DEXfat_pred_stall <- predict(my.data_prune, newdata = test)

compare_data=merge(barres, DEXfat_pred_stall, by="row.names",all.x = TRUE)

write.csv2(compare_data, "mergeR1112.txt")
write.csv2(DEXfat_pred_stall, "predictedR1112.txt")
write.csv2(test$STALL, "testR1112.txt")

DEXfat_pred_stall 

prp(my.data_prune, faclen = 0, cex = 1.0)
